...
void AGameStateManager::Tick(float DeltaTime)
{
	...
	if (len < targetNumVehicles) {
		AAIWheeledVehicle* pVehicle = CreateVehicle(GetActorLocation());
...
AAIWheeledVehicle* AGameStateManager::CreateVehicle(FVector location) {
	UWorld* pWorld = GetWorld();
	...
	AAIWheeledVehicle* SpawnedActor = pWorld->SpawnActor<AAIWheeledVehicle>(vehicleClass, location, rotation, params);
